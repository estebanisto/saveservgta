...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_acceswea:requestDBItems', function(ShopItems)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				TriggerServerEvent('esx_acceswea:buyItem', data.current.item, data.current.value, zone)
			end
...
AddEventHandler('esx_acceswea:hasEnteredMarker', function(zone)
	CurrentAction     = 'shop_menu'
	CurrentActionMsg  = _U('press_menu')
	CurrentActionData = {zone = zone}
end)

AddEventHandler('esx_acceswea:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
...
Citizen.CreateThread(function()
	for k,v in pairs(Config.Zones) do
		for i = 1, #v.Pos, 1 do
			...
			AddTextComponentString(_U('acceswea'))
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('esx_acceswea:hasEnteredMarker', currentZone)
		end
		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_acceswea:hasExitedMarker', LastZone)
		end
	end
end)
...
RegisterNetEvent('esx_acceswea:clipcli')
AddEventHandler('esx_acceswea:clipcli', function()
  ped = GetPlayerPed(-1)
  if IsPedArmed(ped, 4) then
    hash=GetSelectedPedWeapon(ped)
    if hash~=nil then
      TriggerServerEvent('esx_acceswea:remove')
...
RegisterNetEvent('esx_acceswea:silencieux')
AddEventHandler('esx_acceswea:silencieux', function(duration)
...
RegisterNetEvent('esx_acceswea:flashlight')
AddEventHandler('esx_acceswea:flashlight', function(duration)
					local inventory = ESX.GetPlayerData().inventory
				local flashlight = 0
					for i=1, #inventory, 1 do
...
RegisterNetEvent('esx_acceswea:grip')
AddEventHandler('esx_acceswea:grip', function(duration)
					local inventory = ESX.GetPlayerData().inventory
				local grip = 0
					for i=1, #inventory, 1 do
...
RegisterNetEvent('esx_acceswea:yusuf')
AddEventHandler('esx_acceswea:yusuf', function(duration)
					local inventory = ESX.GetPlayerData().inventory
				local yusuf = 0
					for i=1, #inventory, 1 do
